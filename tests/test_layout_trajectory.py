...
def test_grasp_branch_search_is_lazy_bounded_and_backtracks_to_alternative():
    connector = _ScriptedConnector(
        ...
        ik_policy={"position_tolerance_m": .001, "orientation_tolerance_rad": .01},
...
def test_request_scoped_state_cache_and_wall_deadline_are_fail_closed():
    ...
    connector = LayoutTrajectoryConnector(
        ...
        flange_from_virtual_task_tcp=np.eye(4),
        flange_from_physical_contact=np.eye(4),
        ik_policy={"position_tolerance_m": .001, "orientation_tolerance_rad": .01}, collision_margin_m=0.01, contact_tolerance_m=0.0002,
