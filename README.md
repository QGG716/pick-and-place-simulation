...
2026-09-15 已有同一物理世界的真实结果：最高排实际接收 **4/5**，理想送出 **3**。第五箱 `carton_l07_c04` 的接近规划耗尽预算，未进入物理执行；不能解释为不可达。四次物理取放完成与完整资格通过分开统计：实际驱动输出力矩仍不可用，不能以输入 effort 的零值或模型估计代替实测通过。

原始结果和录像见[四箱执行报告](docs/m710_ideal_outfeed_20260915.md)。本轮已实现空载简化、候选软评分、有界前瞻与调度、解析计时和当前环境复用检查：同实际状态的自由路径从 59 节点降到 11，阈值反向从 81 次降到 11；第五箱覆盖 12 个面/roll 家族后仍未恢复完整路径。另一个独立新世界已完成真实“释放→下一箱抓取”片段，间隔 8.55 秒；65 项针对性检查通过。逐项实现、失败和视频入口见[动作质量改进记录](docs/m710_motion_quality_20260915.md)，不追加整排或全量历史回归。

后续录像默认 **640×360、5 fps、正常物理时间**。物理步长、控制、碰撞和接收监测频率保留；渲染像素减少不代表完整仿真同比例加速。旧世界已经结束，新试验使用独立世界标识，不补接旧视频、不跨世界累计完成数。

## 历史说明：以下为此前布局、官方模型初始化及 V3 恢复阶段的记录

下述旧阶段的 `0/5`、尚未搜索和初始间隙阻塞结论不是上述当前结果；相应原始报告继续保留。
