...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--render-every", default=6, type=int)
    ...
    parser.add_argument("--post-release-seconds", default=1.0, type=float)
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.physics_hz <= 0.0 or not math.isfinite(args.physics_hz):
    ...
    if not math.isfinite(args.post_release_seconds) or args.post_release_seconds < 0.0:
...
try:
    ...
    requested_import_settings = {
        "collision_source": "urdf_collision_mesh",
    ...
    if conveyor_start_policy not in {"immediate", "after_release_retreat"}:
        raise ValueError("conveyor start_policy must be immediate or after_release_retreat")
    ...
    for primitive in scene_primitives:
        ...
        if conveyor_enabled and primitive_name in conveyor_directions_world:
            ...
            enabled_attr.Set(conveyor_start_policy == "immediate")
    ...
    if gripper_collision_mesh:
        ...
        suction_tool_xform.SetTranslate(Gf.Vec3d(tool_face_x - 0.01, 0.0, 0.0))
    ...
    if physical_cup_centers.shape == (physical_cup_count, 2):
        ...
        for cup_index, (cup_y, cup_z) in enumerate(physical_cup_centers):
            ...
            cup_xform.SetTranslate(
                Gf.Vec3d(
                    tool_face_x - 0.5 * visual_cup_height,
                    float(cup_y),
                    float(cup_z),
                )
            )
    ...
    gripper_wrench_envelope_complete = bool(
        configured_shear_force is not None and configured_holding_torque is not None
    )
    if target_carton_path is not None and args.gripper_model == "surface_gripper":
        ...
        surface_force_per_point = surface_force / max(physical_active_cup_count, 1)
        # SurfaceGripper reports rigid-constraint impulses, not a compliant
        # pneumatic load distribution. Per-cup catalogue limits caused all
        # constraints to pop on the first solver transient even with a 7 kg
        # payload. Use the active-head aggregate only as a numerical attachment
        # threshold; physical per-cup and total limits remain separate evidence
        # and qualification stays fail-closed without calibrated wrench data.
        surface_solver_force_limit = surface_force
        ...
        surface_shear_force_per_point = surface_shear_force / max(physical_active_cup_count, 1)
        surface_solver_shear_force_limit = surface_shear_force
        ...
        for index, (offset_y, offset_z) in enumerate(point_offsets):
            ...
            ).Set(surface_solver_force_limit)
            surface_gripper_prim.GetAttribute(
                robot_schema.Attributes.SHEAR_FORCE_LIMIT.name
            ).Set(surface_solver_shear_force_limit)
            surface_gripper_prim.GetAttribute(
            ...
            attachment.CreateLocalPos0Attr().Set(
                Gf.Vec3f(tool_face_x, float(offset_y), float(offset_z))
            )
    ...
    for prim, position_attribute, velocity_attribute in (
        (
            root_prim,
        ...
        (
            stage.GetPrimAtPath(target_carton_path) if target_carton_path else None,
    ...
    stiffness = np.asarray([80000.0, 80000.0, 60000.0, 12000.0, 8000.0, 5000.0], dtype=np.float32)[command_order]
    damping = np.asarray([5000.0, 5000.0, 4000.0, 800.0, 500.0, 300.0], dtype=np.float32)[command_order]
    ...
    if args.record_video:
        replay_video_writer = cv2.VideoWriter(
            ...
            float(args.physics_hz / args.render_every),
        ...
        if args.video_preview_speed > 1.0:
            preview_video_writer = cv2.VideoWriter(
                ...
                cv2.VideoWriter_fourcc(*"mp4v"),
                float(args.physics_hz / args.render_every * args.video_preview_speed),
                (args.width, args.height),
            )
    ...
    conveyor_transport_samples: list[tuple[float, np.ndarray, np.ndarray]] = []
    ...
    if conveyor_enabled and conveyor_start_event_time is None:
        raise ValueError("after_release_retreat conveyor requires release_retreat_time_seconds")
    ...
    for step in range(physics_steps):
        ...
        if (
            ...
            for enabled_attr in conveyor_surface_enabled_attrs:
        ...
        if (
            ...
            # Reproduce the SurfaceGripper forward ray geometry explicitly so
            # a failed close can be audited instead of being reported only as
            # an opaque plugin state transition.
            ...
            target_half_extents = 0.5 * np.asarray(target_primitive["size_m"], dtype=float)
            ray_direction_world = body_rotation_matrix @ np.asarray([1.0, 0.0, 0.0])
            ray_direction_local = carton_rotation_matrix.T @ ray_direction_world
            for offset_y, offset_z in point_offsets:
                point_local_body = np.asarray(
                    [tool_face_x, float(offset_y), float(offset_z)], dtype=float
                )
                point_world = body_position + body_rotation_matrix @ point_local_body
                point_local_carton = carton_rotation_matrix.T @ (point_world - carton_position)
                ray_min = -float("inf")
                ray_max = float("inf")
                for axis in range(3):
                    direction = float(ray_direction_local[axis])
                    if abs(direction) < 1e-12:
                        if abs(float(point_local_carton[axis])) > target_half_extents[axis]:
                            ray_min, ray_max = 1.0, 0.0
                            break
                        continue
                    first = (-target_half_extents[axis] - point_local_carton[axis]) / direction
                    second = (target_half_extents[axis] - point_local_carton[axis]) / direction
                    ray_min = max(ray_min, min(first, second))
                    ray_max = min(ray_max, max(first, second))
                distance = max(ray_min, 0.0)
                attachment_raycast_distances_m.append(
                    float(distance) if ray_max >= distance else None
                )
            ...
            if (
                ...
                if elapsed_after_landing <= conveyor_transport_audit_window_s + 1e-9:
                    conveyor_transport_samples.append(
                        (simulation_time, payload_center.copy(), payload_linear_velocity.copy())
                    )
    ...
    if conveyor_initial_direction_world is not None and conveyor_transport_samples:
        ...
        conveyor_direction_matrix = np.unique(
            np.round(np.asarray(list(conveyor_directions_world.values()), dtype=float), 12),
            axis=0,
        )
        transport_deltas = np.diff(transport_positions, axis=0)
        if transport_deltas.size:
            # Follow the piecewise path of an L conveyor. At each sample the
            # physically relevant progress is the largest positive projection
            # onto any configured running belt direction. Projecting the whole
            # trace onto only the release belt makes a correct 90-degree turn
            # look like the carton stopped.
            step_progress = np.maximum(
                0.0, np.max(transport_deltas @ conveyor_direction_matrix.T, axis=1)
            )
        ...
        projected_speeds = np.max(
            transport_velocities @ conveyor_direction_matrix.T, axis=1
        )
        ...
        conveyor_transport_speed_within_tolerance = bool(
            abs(conveyor_transport_projected_speed_m_s - conveyor_speed_m_s)
    ...
    if replay_frames:
        replay_frames[0].save(
            ...
            duration=max(1, int(round(1000.0 * args.render_every / args.physics_hz))),
            loop=0,
            optimize=False,
        )
    ...
    result = {
        ...
        "effective_render_rate_hz": args.physics_hz / args.render_every,
        ...
        "conveyor_transport_engaged": conveyor_transport_engaged,
        "conveyor_transport_speed_within_tolerance": conveyor_transport_speed_within_tolerance,
        "conveyor_transport_speed_audit_model": "piecewise_max_configured_surface_direction",
        ...
        "gripper_attachment_raycast_within_capture_count": sum(
            ...
            and distance <= float(gripper_cfg.get("max_grip_distance_m", 0.03)) + 1e-9
        ...
        "surface_gripper_solver_coaxial_limit_per_constraint_n": (
            float(surface_solver_force_limit)
            if args.gripper_model == "surface_gripper" and surface_gripper_paths
            else None
        ),
        "surface_gripper_solver_shear_limit_per_constraint_n": (
            float(surface_solver_shear_force_limit)
            if args.gripper_model == "surface_gripper" and surface_gripper_paths
            else None
        ),
        "surface_gripper_solver_limits_are_physical_per_cup_limits": False,
