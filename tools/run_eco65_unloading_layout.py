...
from unloading_sim.eco65.unloading_layout import CONFIG,build_scene,audit_layout,UnloadingWorld,limited_reachability,source_mapping

def digest(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('stage',choices=['build','poses','render','export','verify'])
    ...
    if args.stage=='build':
        ...
        c=yaml.safe_load(CONFIG.read_text(encoding='utf-8'));cad=asset_check();tool=load_json(ROOT/c['robot']['private_tool_config']);scene=build_scene(c)
        ...
        snapshot=dict(scene=scene,tool=tool,config_sha256=digest(CONFIG),asset_sha256=fingerprints)
        ...
        return
    snapshot=load_json(out/'scene_snapshot.json');claimed=snapshot.pop('fingerprint');assert claimed==canonical_fingerprint(snapshot);snapshot['fingerprint']=claimed
