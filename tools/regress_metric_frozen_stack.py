...
from unloading_contracts import to_wire,loads
...
def main():
    ...
    p.add_argument('--report-only',action='store_true',help='Rebuild paired reports from persisted final geometry/observations; never rerun fitting')
    args=p.parse_args()
    ...
    args.output.mkdir(parents=True,exist_ok=args.report_only)
    ...
    for folder in (root,root/'modules/module_1_lower'):
        ...
        output=args.output/scene/camera['module_id']; output.mkdir(parents=True,exist_ok=args.report_only)
        ...
        if args.report_only:
            final=json.loads((output/'validated_geometry.json').read_text())
            obs=loads((output/'observation.json').read_text())
            faces=[]
            for cargo in obs.cargo:
                data=to_wire(cargo.raw_result['observed_face_set'])
                data['faces']=tuple(ObservedFace(**f) for f in data['faces'])
                faces.append(ObservedFaceSet(**data))
        else:
        ...
        if not args.report_only: (output/'observation.json').write_text(dumps(obs))
    ...
    (args.output/'paired_stack_report.json').write_text(json.dumps(to_wire(summary),indent=2))
