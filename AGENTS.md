# ECO65-B desktop — BOOTSTRAP

当前项目是 ECO65-B 桌面原理验证样机；目标分支 feat/v0.6-eco65-desktop。
实机型号 ECO65-B 已由用户确认；控制器代际仍待确认。
当前授权仅为本地源码、独立 CPU 环境、官方原始资料和真实 STEP 投放目录准备。
ECO65 功能尚未适配，三个来源尚未集成。来源固定于 configs/integration/eco65_desktop_sources.lock.yaml。
准备完成立即停止，等待用户真实 STEP 和下一条明确继续指令。文件到达不构成继续授权。
禁止监听、后台任务、STEP 解析/网格化/OBB、假想吸具、模型适配、规划、仿真、回放和实机连接。
实机限制包括只读连接、IP 扫描、使能、参数和 IO/真空操作。
仅运行准备/导入/文件完整性检查；不运行全量 pytest、104/129 验收或抓取任务。
保持 SI 单位、可复现性、确定性随机种子、模块隔离和原始资料保护；重型依赖保持可选。
吸具质量、材质、TCP、法兰关系、吸盘数量、真空气路均 unknown/null。
1000W 铭牌不足以确认供电兼容，本轮不进行供电测试。
保留全部 FANUC 历史配置、功能、证据。下面旧指令仅作历史，不是当前生效要求。
不得沿用 FANUC 布局、72 杯吸具、机器人参数、碰撞豁免、服务器路径或仅推旧分支要求。
允许本地 bootstrap 提交；不推送，不公开用户 CAD。实际状态见 docs/eco65_desktop_bootstrap.md。

---

# 以下为非生效的 FANUC 历史开发指令
...
- Do not globally reduce collision margins.  Preserve all checks except explicit J5/J6 versus owned-tool simulation exemptions, the user-approved flexible-cup/non-target-stack acceptance exception, and stage-scoped target/stack planning contact relaxation.  Tool/environment, wrist/environment and other robot/tool pairs remain checked.
