# ECO65-B desktop — COMPACT_SINGLE_TASK

当前主线为四箱紧凑布局与一个上层箱完整几何抓放，随后仅该箱 IDEAL_OUTFEED。
1400×900 mm 为设计约束，真实办公桌未测量。模型和工具 1:1；完整厢体、另外三箱与全部附件参与碰撞。
默认 configs/workcells/eco65_desktop_unloading_compact_v1.yaml；保留九箱配置和全部历史证据，不覆盖旧 run_id。
使用已锁定的三来源，不整体合并、不更新来源锁。只修改 feat/v0.6-eco65-desktop。
先展示布局，继续规划、独立验证与执行，不等待再次授权。仅针对性测试，不整堆、不全量 pytest、不性能扫描。
所有硬件动作禁用；无视觉、ROS 或厂家 SDK。几何回放不称为物理成功，Isaac 不可用标 NOT_EVALUATED。
气路未知，全部真实吸附轮廓必须有效；不继承 FANUC 工具/邻箱豁免，不减小碰撞余量。
规划、验证、执行使用同一初始快照的独立世界；完成数量来自实际事件。
STEP、网格、工具配置、详细快照、图片与视频只保存在本地忽略目录。用户授权提交并推送公开代码、布局配置和报告，不强推。
本轮一箱结束后停止，不继续其他三箱或真机工作。
