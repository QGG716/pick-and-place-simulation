# ECO65-B desktop — ROUND1_ADAPTATION

用户已明确授权第一轮正式适配：真实 STEP 解析、装配/TCP、官方 ECO65-B 运动学、桌面单箱规划、完整碰撞验证和本地几何回放。
仅本地工作，目标分支 feat/v0.6-eco65-desktop；沿用已锁定三来源，不整体 merge。
实机型号 ECO65-B 已由用户确认；控制器代际、固件与物理法兰修订仍待确认。
所有硬件动作保持禁用，包括只读连接、IP 扫描、厂家 SDK、IO/真空；enable_hardware=true 必须拒绝。
本轮允许按实际 CAD 推导及明确的仿真安装/质量假设继续，不要求重复授权。
raw 原件只读；派生网格、细节清单、装配图和完整场景仅本地私有，不上传，不提交。
SI 单位，T_A_B 把 B 表达变为 A 表达；缩放与刚体变换分离；全链路统一完整 TCP 变换。
同一进程只有本分支的一套 unloading_sim。CAD/ROS/Isaac/厂家 SDK 可选，不耦合到 CPU 核心。
仅相关验证；不运行全量 pytest、104/129 历史任务、整堆或无关扫描。
首条完整有效路径获得后交付回放，不继续性能优化；Isaac 不可用时明确 GEOMETRIC_REPLAY_ONLY / NOT_EVALUATED。
保留全部 FANUC 历史，不沿用其布局、72 杯规则、参数、碰撞豁免、服务器或推送指令。
允许当前分支本地提交，不推送。当前详细要求由本轮用户请求定义。
