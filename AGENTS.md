...
9. The latest requested run attempts the complete current highest row, deriving its size from the scene. Keep verification minimal and retain unsuccessful physical attempts as well as successes. Use the optional 1080p/15 fps recording configuration to reduce rendering wall time while preserving 1x physical time and every runtime gate.
