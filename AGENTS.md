# ECO65-B desktop — UNLOADING_LAYOUT_DESIGN

当前任务是与固定 feasibility 卸货工作站拓扑一致的桌面布局 DESIGN_CANDIDATE：完整厢体、9 箱、L 形双带、固定 ECO65-B 与真实吸具。机器人/工具保持 1:1，无底盘或升降。
保留上一轮单箱配置、所有私有资产和验证证据；新布局使用独立配置/快照/run_id，不覆盖旧结果。
来源布局只读 9db77cb8a51e9bf1821c6e84e90a7632fbce6b26，不更新三来源锁，不合并其他分支。
本轮仅布局、工程图、初始状态和少量代表性姿态 IK/完整碰撞检查；不搜索完整路径、不整堆、不节拍优化、不全量测试。
左右墙、后墙、底板与顶板均参与碰撞；隐藏显示不得移除碰撞。全部邻箱/下层支撑保留，气路未知，不假设独立吸盘控制。
所有硬件动作禁用，enable_hardware=true 必须拒绝。无相机、ROS、厂家 SDK、远端仿真或资产上传。
完整 TCP/安装关系沿用，安装转接架仍为仿真假设。SI 单位；T_A_B 把 B 表达变为 A 表达。
桌面实物尺寸未知，输出最小静态占地、操作/安装预留空间和待实测状态，不称为加工图。
只修改 feat/v0.6-eco65-desktop；用户明确授权本轮完成后提交并推送公开代码/配置模板/说明，不强推。
STEP、派生网格、详细工具配置、完整快照、图片和视频保持本地 Git 忽略。布局图与有限检查完成即交付，等待用户确认尺寸方向。
