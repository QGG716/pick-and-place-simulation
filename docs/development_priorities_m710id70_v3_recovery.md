...
> **2026-09-14 动作策略更新**：直接接近与自适应预抓取共用真实接触门控；
> 预抓取、固定上抬与固定撤离不再是必选阶段。`TRANSVERSE_SIDE` 工作法向为世界 +X。
> 支撑释放与有界短落释放分别验收，预测落带不得冒充当前支撑或实际接收。
> 已接收箱体在续箱期间保留物理实体、移动占用和接收监测。
> 本轮实现、实际执行及复现入口见 [自适应连续取放记录](m710_adaptive_cycle_20260914.md)。

> **本轮有效策略（2026-09-10，用户最新授权）**：CPU、Isaac 与结果汇总共同使用
> `configs/validation/m710id70_layout_v1.yaml` 中的具名、分阶段碰撞策略。
> 仅 `J5_link/J6_link`—自有吸具碰撞对记为 `USER_APPROVED_SIMULATION_EXEMPTION`；
> 脱垛阶段目标—原堆垛箱采用 `planner_relaxed_physics_checked`，实际箱间碰撞保持开启，
> 由真实进展、穿透和邻箱扰动门控；实际离垛后恢复工程净空。其他碰撞余量不变。
> CAD 自动复核已恢复 72 个刚性嵌件：202 实体分为 130 刚体和 72 柔性杯体，
> 向外包含证据见 `assets/grippers/shanghai_wantai_three_zone/geometry_coverage.json`。
> 当前优先交付实际首箱取放录像，然后在同一物理世界按实际剩余状态逐行续箱。
> 吸附容量为用户认可的仿真假设，不等同真机安全放行。本轮只做聚焦回归，
> 不默认运行历史 104/129 全量。以下旧门控作为历史发现保留，不再覆盖本段有效策略。
...
1. 统一具名腕部豁免、允许物理接触的脱垛和 CAD 实体覆盖表示。
2. 从实际最高未完成行中心优先搜索，给替代面和两条传送带真实的有界预算。
3. 通过完整几何轨迹后，执行实际接触、附着、脱垛、搬运、支撑放置、解除约束和撤离。
4. 首次完整结果立即保存录像与实际状态；保持同一 World，基于实际箱姿态和带面占用续箱。
5. 聚焦回归、短硬编码清单和结果记录；所有仿真假设与未完成项如实区分。
...
状态：上一轮历史顺序不再是当前阻塞依据；本轮以本文顶部有效策略为准。冻结的
