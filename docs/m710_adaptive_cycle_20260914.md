...
本次连续尝试的范围是当前最高排 5 箱，数量由初始场景任务集合读取；下方 35 箱继续留在物理世界中。

## 最新结果：2/5 完成，第三箱接近时停止

同一个 Isaac World 实际执行了三段：前两箱释放并接收成功，第三箱未到抓取就触发接近保护。
最上排尚余 `carton_l07_c03`、`carton_l07_c04`、`carton_l07_c00`；后两箱没有物理执行。
全部 40 个刚体保留，已交接出工作单元数量为 0。

| 箱体 | 规划墙钟 / s | 实际动作 / s | Isaac 墙钟 / s | 结果 |
| --- | ---: | ---: | ---: | --- |
| carton_l07_c02 | 120.632（复用） | 90.433 | 1114.165 | 短落 24.135 mm，接收成功 |
| carton_l07_c01 | 237.713 | 64.883 | 576.624 | 短落 22.960 mm，接收成功，纵带已停 |
| carton_l07_c03 | 643.048 | 4.950 | 14.058 | 接近保护停止，未抓取、未释放 |

第三段在 `pregrasp`、未附着状态下，`CupCompressedCollision_05` 与已接收的 `carton_l07_c01`
产生 `UNEXPECTED_ROBOT_OR_RIGID_TOOL_PROXIMITY`。报告最小分离距离为 **19.985631 mm**，
是正间隙接近事件，不能称为已发生穿透。该段最大关节跟踪误差为 0.000728 rad。
没有放宽碰撞门槛或删除已接收箱体来继续执行；本轮保留停止结果。

交付目录为本地 `outputs/m710_adaptive_20260914/` 和服务器
`/root/autodl-tmp/m710-adaptive-20260914/first_row_delivery/`：

- `continuous_first_row.mp4`：两箱成功和第三箱停止前动作，1080p、15 fps、1×，2403 帧，160.2 s。
- `segment_003.mp4`：第三箱的 4.93 s 实际录像；`last_recorded_frame.jpg` 是停止前最后录制帧。
- `summary.json`、`third_attempt_result.json`、`actual_remaining_state.json`：计数、具体事件及全部实际箱体状态。

连续视频帧数等于三段原始视频帧数之和，首尾帧可读取；总物理动作时间为 160.267 s，
与视频时长相差约一个采样帧。离线规划暂停不计入视频时间，画面中的仿真时钟连续。
旧成功录像、旧接收后偏转失败录像分别保留，没有混入本次卸箱数量。
前两箱实际选择直接接近、单段直线脱垛和短落释放；未选用非直线脱垛。
释放后均无静止等待，分别采用 30.219 mm、30.216 mm 的已检查脱离路线；第三段的接近保护停止说明后续实际净空仍有未解决项。
...
第二箱 `carton_l07_c01` 在同一个 World 完成，计划起点与首箱实际结束 q 的最大差为 0。
规划 237.713 s，实际动作 64.883 s，Isaac 墙钟 576.624 s，实际释放和接收均成功。
首箱在第二段 6.8 s 时到达出口保护范围（余量 93.634 mm），纵带按明确事件停带，未重置箱体速度。
第二箱接收后也停留在该带面；因此第二段的输送参与及输送速度指标未通过，不能报告为已出料。
`physical_cycle_completed` 与这些输送指标分别保留，没有把资格总布尔值改成成功。
...
前述独立首箱成功试验中，`qualification_passed=false` 的唯一剩余项目是实际驱动输出力矩通道不可用（`NOT_EVALUATED`）。
