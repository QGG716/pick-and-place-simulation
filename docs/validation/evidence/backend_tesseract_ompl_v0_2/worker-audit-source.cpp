...
struct Context {
  ...
  bool valid(const Eigen::VectorXd& qv) {
    ...
    if(stopped()) {++rejections[termination];return false;}
    ++states;
    ...
    auto transform_start=Clock::now();
...
struct DenseMotion final : ob::MotionValidator {
  ...
  bool check(const ob::State* a,const ob::State* b,std::pair<ob::State*,double>* last) const {
    Profile::Scope timer(ctx.profile,"edge_check_inclusive_s");
    ++ctx.edges;
    ...
    for(size_t i=0;i<=n;++i) {
      ...
      if(!ctx.valid(sample)) {
        if(ctx.termination.empty()) {++ctx.invalid_edges;++ctx.rejections[ctx.failure.value("reason",std::string("UNKNOWN"))];}
        else ++ctx.incomplete_edges;
        if(last) {
  ...
  if(audit) {
    if(r.contains("states")) {
      ...
      for(const auto& state:r.at("states")) {
        ...
        if(!accepted && !result.contains("first_rejection")) result["first_rejection"]={{"q_rad",state},{"failure",ctx.failure}};
