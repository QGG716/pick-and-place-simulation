...
struct Context {
  ...
  uint64_t states=0, edges=0, samples=0, max_states=0, termination_checks=0;
...
json solve(Context& ctx,const json& r) {
  ...
  json result={{"candidate_found",false},{"exact_solution",false},{"native_validated",false},{"path",json::array()},
    {"versions",{{"tesseract","0.35.0"},{"tesseract_planning","0.35.0 (installed; pipeline bypassed)"},
      {"ompl",OMPL_VERSION},{"manager","FCLDiscreteBVHManager"},{"implementation","native C++17 worker"}}}};
  ...
  ctx.states=ctx.edges=ctx.samples=ctx.termination_checks=0;ctx.termination.clear();ctx.failure=nullptr;
  ...
  if(status.empty()) {
    ...
    if(clear) {
      ...
      if(solved==ob::PlannerStatus::EXACT_SOLUTION && problem->hasExactSolution()) {
        ...
        status="CANDIDATE";
      } else status=ctx.termination.empty()?"BUDGET_EXHAUSTED":ctx.termination;
    }
  }
  if(status=="CANDIDATE") {
  ...
  result["counters"]={{"state_checks",ctx.states},{"edge_checks",ctx.edges},{"subdivision_samples",ctx.samples},
                      {"termination_checks",ctx.termination_checks}};
