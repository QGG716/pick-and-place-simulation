...
## Stage 4 — load and executable timing

Replaced discrete derivative timing with C2 rest-to-rest quintic edges. The
positions trace the same collision-checked joint edges; analytic derivative
extrema bound velocity, acceleration and jerk. A 1 rad single-joint move
with acceleration <=1 rad/s² must take at least 2 s; the actual quintic takes
2.402811 s. Window/driver retiming uses the same interpolation and evaluates
torque callbacks at interior states, not only stationary knots.

External load uses the same captured box transform as collision, full inertia
rotation, tensor parallel-axis shifts, actual joint axes and Newton-Euler
external wrenches. A tensor-product Gauss quadrature independently verifies
the rotated box inertia. Wrist references are not motor peak torques. The
baseline front-grasp CoM still fails even the 60 kg curve's larger axial
intercept; points within the intercept rectangle are NOT_EVALUATED, not PASS.
Missing suction evidence, complete robot inertials and drive limits remain
NOT_EVALUATED and cannot erase a known FAIL.

Full default suite after receiver and inertia regressions: 237 passed,
1 deselected (repository default simulation marker). Independent numeric
evidence generated by `tools/audit_m710_v3.py`.

## Stage 5 — capability experiments on the corrected contract

The V3 entry point is `tools/run_m710id70_v3.py`; the public
`tools/run_m710id70_acceptance.py` CLI now forwards to it. Historical V2
helpers remain for fixtures, but exact V2 replay uses
`tools/reproduce_m710_v2.py` and the immutable reference commit, because
shared math fixes intentionally change current imports.

All 104 original valid grid samples are paired fixed/dynamic conveyor tasks
with identical planner, home, seed, collision margin, IK precision and path
integration. Report new/lost/common feasible sets separately; compute path
and cycle deltas only on common feasible tasks. Lift analysis evaluates the
same 104 tasks at each of five explicit heights and validates the intervening
lift motion; it reports coverage union, not an inferred zero stroke.

Extraction logs name every local constraint and the selected direction and
distance. Side candidates include 0/90/180/270° roll with individual complete
cup checks. A controlled fixed-belt bottom-box attempt tests an alternative
to the dynamic-belt upper/lower-height conflict without weakening collision.

Independent task processes use deterministic per-task seeds and return in
...
The exhaustive height run also exposed wasted search before a deterministically
invalid attached initial state: a 10 mm neighbor gap cannot satisfy two 10 mm
OBB inflations. This unchanged predicate is now checked immediately after
